...
- `compose.yaml` — container definitions (database, Strapi, optional nginx).
- `myd17.sh` — primary operator script for installation, updates, backups, restores and diagnostics.
- `package.json` `onprem` script — forwards to `myd17.sh` for developers who use `pnpm onprem`.
- `nginx/conf.d/myd17.conf` — nginx config generated by `./myd17.sh install` (operator-editable).
- `nginx/ssl/` — SSL certificate directory (operator-managed).
...
- Check `./myd17.sh status`.
- Check `./myd17.sh logs strapi`.
